...
#Read netcdf
file_chl = "/net/fluo/data3/data/FluoData1/students/renato/aviris_dangermond/traits/chl_aviris_dangermond_time_00_reg.nc";
chls_1 = read_nc(FT, file_chl, "chl");
#chls = chls_1[1600:1611,1600:1611,1];
chls = chls_1[1:492,1:458,1];
#chls = chls_1[1:201,1:201,1];

#lais = 0.2 .+ rand(12,12) .* 3;
file_lai = "/net/fluo/data3/data/FluoData1/students/renato/aviris_dangermond/traits/lai_aviris_dangermond_time_00_reg.nc";
lais_1 = read_nc(FT, file_lai, "lai");
#lais = lais_1[1600:1611,1600:1611,1];
lais = lais_1[1:492,1:458,1];


#vcms = 20 .+ rand(12,12) .* 30;

file_vcm = "/net/fluo/data3/data/FluoData1/students/renato/aviris_dangermond/traits/lma_aviris_dangermond_time_00_reg.nc";
vcms_1 = read_nc(FT, file_vcm, "lma");
#vcms = vcms_1[1600:1611,1600:1611,1];
vcms = vcms_1[1:492,1:458,1];

#chls[1] = NaN;
gpps,sifs740,sifs683,sifs757,sifs771,transps = run_one_timestep!(day, chls, lais, vcms);

#ncresult::String = "shift_fluxes_day_00_reg_test8.nc"

#_len_lat = size(vcms, 2);
#_len_lon = size(vcms, 1);
...
ncresult::String = "shift_fluxes_day_00_reg.nc"

lats_1 = read_nc(FT, file_vcm, "lat");
#lats = lats_1[1:201];
lats = lats_1[1:458];
lons_1 = read_nc(FT, file_vcm, "lon");
#lons = lons_1[1:201];
lons = lons_1[1:492];
...
append_nc!(ncresult, "gpp", gpps, Dict("gpp" => "gpp"), ["lon", "lat"]);
append_nc!(ncresult, "sif740", sifs740, Dict("sif740" => "sif740"), ["lon", "lat"]);
append_nc!(ncresult, "sif683", sifs683, Dict("sif683" => "sif683"), ["lon", "lat"]);
append_nc!(ncresult, "sif757", sifs757, Dict("sif757" => "sif757"), ["lon", "lat"]);
append_nc!(ncresult, "sif771", sifs771, Dict("sif771" => "sif771"), ["lon", "lat"]);
append_nc!(ncresult, "transp", transps, Dict("transp" => "transp"), ["lon", "lat"]);
