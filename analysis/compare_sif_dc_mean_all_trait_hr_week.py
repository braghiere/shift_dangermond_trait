...
try:
    file = open('mean_sif_dc_trait_statistics_hr.txt', 'w')
...
for i, target_date_str in enumerate(dates):
    ...
    valid_indices = ~np.isnan(ds2_selected['sif_dc'].values.flatten()) & ~np.isnan(ds1_selected.values.flatten())
    if valid_indices.any():
        observed = ds2_selected['sif_dc'].values.flatten()[valid_indices]
        ...
        # Calculate spatial differences
        spatial_diff =  ds2_selected['sif_dc'].values
        ...
        cs = m.pcolormesh(X, Y, spatial_diff, cmap='viridis', vmin=-0.2, vmax=1.75)
        ...
        plt.title(f'SIF_dc at {target_date_str}',fontsize=14)
        ...
        plt.tight_layout()
        # Save the figure
        plt.savefig(f'sif_dc_{i}_hr.png',dpi=300)
        #plt.show()
        #sys.exit()
        ...
        # Filter out NaN values
        valid_indices = ~np.isnan(ds1_selected.values) & ~np.isnan(ds2_selected['sif_dc'].values)
        observed = ds2_selected['sif_dc'].values.reshape(-1)[valid_indices.reshape(-1)]
...
# Plot spatial differences using pcolormesh with Basemap
cs = m.pcolormesh(X, Y, average_spatial_diff, cmap='viridis', vmin=-0.2, vmax=1.75)
...
plt.tight_layout()
# Save the figure
plt.savefig(f'sif_dc_mean_hr.png', dpi=300)
