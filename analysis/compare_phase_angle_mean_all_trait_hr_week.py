...
# Open the NetCDF file for TROPOMI dataset _clipped_458_492
...
try:
    file = open('mean_phase_angle_trait_statistics_hr.txt', 'w')
...
for i, target_date_str in enumerate(dates):
    ...
    valid_indices = ~np.isnan(ds2_selected['phase_angle'].values.flatten()) & ~np.isnan(ds1_selected.values.flatten())
    if valid_indices.any():
        observed = ds2_selected['phase_angle'].values.flatten()[valid_indices]
        ...
        # Calculate spatial differences
        spatial_diff =  ds2_selected['phase_angle'].values
        ...
        cs = m.pcolormesh(X, Y, spatial_diff, cmap='viridis', vmin=-70, vmax=0)

        vmin = -70
        vmax = 0
        cbar = m.colorbar(cs, pad='10%',ticks=np.linspace(vmin,vmax,7),format='%.2f')
        cbar.ax.get_yaxis().labelpad = 10
        cbar.ax.set_ylabel(r'Phase Angle (deg)', rotation=270, verticalalignment='center', color='black', size=16)
        ...
        plt.title(f'Phase Angle at {target_date_str}',fontsize=14)
        ...
        plt.tight_layout()
        # Save the figure
        plt.savefig(f'phase_angle_{i}_hr.png',dpi=300)
        #plt.show()
        #sys.exit()
        ...
        # Filter out NaN values
        valid_indices = ~np.isnan(ds1_selected.values) & ~np.isnan(ds2_selected['phase_angle'].values)
        observed = ds2_selected['phase_angle'].values.reshape(-1)[valid_indices.reshape(-1)]
...
# Plot spatial differences using pcolormesh with Basemap
cs = m.pcolormesh(X, Y, average_spatial_diff, cmap='viridis', vmin=-70, vmax=0)
...
plt.tight_layout()
# Save the figure
plt.savefig(f'phase_angle_mean_hr.png', dpi=300)
