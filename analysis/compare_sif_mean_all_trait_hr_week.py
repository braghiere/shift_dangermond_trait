...
try:
    file = open('mean_sif_trait_statistics_hr.txt', 'w')
...
for i, target_date_str in enumerate(dates):
    ...
    if valid_indices.any():
        ...
        # Calculate spatial differences
        spatial_diff =  ds2_selected['sif'].values
        ...
        plt.title(f'SIF at {target_date_str}',fontsize=14)
        ...
        plt.tight_layout()
        # Save the figure
        plt.savefig(f'sif_{i}_hr.png',dpi=300)
        #plt.show()
        #sys.exit()
...
plt.tight_layout()
# Save the figure
plt.savefig(f'sif_mean_hr.png', dpi=300)
